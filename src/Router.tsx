...
export default ({ scrollUp, url: baseUrl, children }: RouterProps) => {
  const init = baseUrl || (isServer() ? "/" : window.location.href);
  const [url, setStateUrl] = useState<Url>(() => parse(init));
  ...
  const setUrl = useCallback(
    (newUrl: NewUrlValue | ((prev: Url) => NewUrlValue), { mode = "push" }: SetUrlOptions = {}) => {
      ...
      setStateUrl((prev) => {
        ...
        if (stringify(prev) === stringify(resolved)) return prev;
      ...
      if (stringify(prev) !== stringify(resolved)) {
        ...
        urlRef.current = next;
        (history as any)[mode + "State"]({}, null, stringify(next));
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      const href = getHref(target?.closest("a") ?? null);
