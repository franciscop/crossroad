...
export default function samePath(
  ...
  if (refParsed.path.endsWith("*")) {
    ...
    urlParsed.path =
      "/" + urlParsed.path.slice(1).split("/").slice(0, length).join("/");
  }
  ...
  if (Object.entries(refParsed.query).length) {
    for (const key in refParsed.query) {
      ...
      if (refParsed.query[key] && refParsed.query[key] !== urlParsed.query[key])
        return false;
    }
  }

  // Straightforward comparison between paths when there are no parameters
  if (!refParsed.path.includes(":"))
    return refParsed.path === urlParsed.path && params;

  // Different length, cannot be the same paths
  if (refParsed.path.split("/").length !== urlParsed.path.split("/").length)
    return false;

  // Make a comparison per-parameter; and extract those parameters
  const extra: Params = {};
  const match = refParsed.path
    .split("/")
    .every((refPart: string, i: number) => {
